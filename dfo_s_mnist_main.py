...
def main():
    ...
    if target_label is not None:
        ...
        plt.subplot(1, 2, 1)
        plt.imshow(x0.squeeze().numpy(), cmap='gray')
        plt.title(f'Original (Pred: {pred_label})')
        plt.axis('off')
        
        plt.subplot(1, 2, 2)
        plt.imshow(counterfactual_image.squeeze().detach().numpy(), cmap='gray')
